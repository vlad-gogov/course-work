...
class ModeServiceDevice(ModeService):
    ...
    def service(self, flow_cars: Flow, start_time: float = 0, delta: float = 0):
        ...
        debug_log("Число обслуживаемых машин: ", l)
        for i in range(l):
            if t <= start_time + new_time_work:
                ...
                # TODO
                elif start_time + new_time_work - self.time_service <= flow_cars.cars[0] <= start_time + new_time_work:
                    debug_log("Обслуживание последних скопившихся машин: ")
                    debug_log(i, ":", flow_cars.cars[:l - i])
